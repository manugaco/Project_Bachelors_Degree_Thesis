...
cv_glmnet #Accuracy 95.8% (overfitted)

#Making the models to test the accuracy in split mode

models = train_models(container, algorithms=c("SVM", "MAXENT", "GLMNET"), verbose = TRUE)
...
table(as.numeric(as.factor(alg_tweets[16001:20000, 2])), results[,"GLMNET_LABEL"])
...
recall_accuracy(as.numeric(as.factor(alg_tweets[16001:20000, 2])), results[,"GLMNET_LABEL"]) #

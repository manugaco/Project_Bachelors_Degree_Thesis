...
UsersDF_FR1_L2 <- UsersDFC_FR_01 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR1_L2, file = "Objects/UDCFR1_L2.RData")

print("1/20 done")
print("2/20 downloading")

UsersDFC_FR_02 <- as.data.frame(sapply(Viplist02, fun_UFR_clean))
UsersDF_FR2_L2 <- UsersDFC_FR_02 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR2_L2, file = "Objects/UDCFR2_L2.RData")

print("2/20 done")
print("3/20 downloading")

UsersDFC_FR_03 <- as.data.frame(sapply(Viplist03, fun_UFR_clean))
UsersDF_FR3_L2 <- UsersDFC_FR_03 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR3_L2, file = "Objects/UDCFR3_L2.RData")

print("3/20 done")
print("4/20 downloading")

UsersDFC_FR_04 <- as.data.frame(sapply(Viplist04, fun_UFR_clean))
UsersDF_FR4_L2 <- UsersDFC_FR_04 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR4_L2, file = "Objects/UDCFR4_L2.RData")

print("4/20 done")
print("5/20 downloading")

UsersDFC_FR_05 <- as.data.frame(sapply(Viplist05, fun_UFR_clean))
UsersDF_FR5_L2 <- UsersDFC_FR_05 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR5_L2, file = "Objects/UDCFR5_L2.RData")

print("5/20 done")
print("6/20 downloading")

UsersDFC_FR_06 <- as.data.frame(sapply(Viplist06, fun_UFR_clean))
UsersDF_FR6_L2 <- UsersDFC_FR_06 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR6_L2, file = "Objects/UDCFR6_L2.RData")

print("6/20 done")
print("7/20 downloading")

UsersDFC_FR_07 <- as.data.frame(sapply(Viplist07, fun_UFR_clean))
UsersDF_FR7_L2 <- UsersDFC_FR_07 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR7_L2, file = "Objects/UDCFR7_L2.RData")

print("7/20 done")
print("8/20 downloading")

UsersDFC_FR_08 <- as.data.frame(sapply(Viplist08, fun_UFR_clean))
UsersDF_FR8_L2 <- UsersDFC_FR_08 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR8_L2, file = "Objects/UDCFR8_L2.RData")

print("8/20 done")
print("9/20 downloading")

UsersDFC_FR_09 <- as.data.frame(sapply(Viplist09, fun_UFR_clean))
UsersDF_FR9_L2 <- UsersDFC_FR_09 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR9_L2, file = "Objects/UDCFR9_L2.RData")

print("9/20 done")
print("10/20 downloading")

UsersDFC_FR_010 <- as.data.frame(sapply(Viplist010, fun_UFR_clean))
UsersDF_FR10_L2 <- UsersDFC_FR_010 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR10_L2, file = "Objects/UDCFR10_L2.RData")

print("10/20 done")
print("11/20 downloading")

UsersDFC_FR_011 <- as.data.frame(sapply(Viplist011, fun_UFR_clean))
UsersDF_FR11_L2 <- UsersDFC_FR_011 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR11_L2, file = "Objects/UDCFR11_L2.RData")

print("11/20 done")
print("12/20 downloading")

UsersDFC_FR_012 <- as.data.frame(sapply(Viplist012, fun_UFR_clean))
UsersDF_FR12_L2 <- UsersDFC_FR_012 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR12_L2, file = "Objects/UDCFR12_L2.RData")

print("12/20 done")
print("13/20 downloading")

UsersDFC_FR_013 <- as.data.frame(sapply(Viplist013, fun_UFR_clean))
UsersDF_FR13_L2 <- UsersDFC_FR_013 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR13_L2, file = "Objects/UDCFR13_L2.RData")

print("13/20 done")
print("14/20 downloading")

UsersDFC_FR_014 <- as.data.frame(sapply(Viplist014, fun_UFR_clean))
UsersDF_FR14_L2 <- UsersDFC_FR_014 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR14_L2, file = "Objects/UDCFR14_L2.RData")

print("14/20 done")
print("15/20 downloading")

UsersDFC_FR_015 <- as.data.frame(sapply(Viplist015, fun_UFR_clean))
UsersDF_FR15_L2 <- UsersDFC_FR_015 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR15_L2, file = "Objects/UDCFR15_L2.RData")

print("15/20 done")
print("16/20 downloading")

UsersDFC_FR_016 <- as.data.frame(sapply(Viplist016, fun_UFR_clean))
UsersDF_FR16_L2 <- UsersDFC_FR_016 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR16_L2, file = "Objects/UDCFR16_L2.RData")

print("16/20 done")
print("17/20 downloading")

UsersDFC_FR_017 <- as.data.frame(sapply(Viplist017, fun_UFR_clean))
UsersDF_FR17_L2 <- UsersDFC_FR_017 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR17_L2, file = "Objects/UDCFR17_L2.RData")

print("17/20 done")
print("18/20 downloading")

UsersDFC_FR_018 <- as.data.frame(sapply(Viplist018, fun_UFR_clean))
UsersDF_FR18_L2 <- UsersDFC_FR_018 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR18_L2, file = "Objects/UDCFR18_L2.RData")

print("18/20 done")
print("19/20 downloading")

UsersDFC_FR_019 <- as.data.frame(sapply(Viplist019, fun_UFR_clean))
UsersDF_FR19_L2 <- UsersDFC_FR_019 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR19_L2, file = "Objects/UDCFR19_L2.RData")

print("19/20 done")
print("20/20 downloading")

UsersDFC_FR_020 <- as.data.frame(sapply(Viplist020, fun_UFR_clean))
UsersDF_FR29_L2 <- UsersDFC_FR_020 %>%
  map(extract) %>%
  map_df(bind_rows) %>%
  dplyr:::select(all)

save(UsersDF_FR20_L2, file = "Objects/UDCFR20_L2.RData")
